...
- **用户正式使用目录已迁移（2026-09-08）**：`D:\日历待办工具\`（v0.2.0 exe + 生产数据 data\，78 条事项）。**今后发新版只需把新 exe 交付给该目录替换**；项目 `src-tauri\target\release\` 下的 v0.1.0/v0.2.0 目录均为旧副本（v0.1.0 data 是过期快照），清理须用户确认。
