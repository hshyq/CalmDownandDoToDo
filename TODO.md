...
**2026-09-14 事项弹窗点外关闭（待提交/待手测）**：点弹窗外·窗口内=关闭不保存（等同取消）；点窗口外=行为不变；仅事项弹窗生效，删除确认点外退回编辑（TC-IT-014/015，方案经用户确认）。`Modal` 基座加 `closeOnOverlayClick`；原型 `modal()` 同步 `dismissable`。vitest 45、build 通过。

**2026-09-14 v0.3.2 已发布到正式目录**（`release\日历待办工具 v0.3.2\`，生产 data 原地未动）：
