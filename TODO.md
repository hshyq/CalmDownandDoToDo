...
**2026-09-08 v0.2.2 已发布到正式目录**（`release\日历待办工具 v0.2.2\`，data 原地未动）：总览页开放工具栏「+ 新增事项」（所属分类下拉，PRD v1.10 / TC-CL-007），移除「不可新增」标签。待用户手测。## 已完成
...
- **用户正式使用目录（2026-09-08 迁移，当前 v0.2.2）**：`release\日历待办工具 v0.2.2\`（项目根下 release\ 目录，生产数据 data\，78 条事项；.gitignore 已排除 release\）。**今后发新版：新版本目录放 `release\` 下，或仅把新 exe 交付用户替换**；`src-tauri\target\release\` 下的 v0.1.0/v0.2.0 目录均为旧副本（v0.1.0 data 是过期快照），清理须用户确认。
