...
**2026-09-08 v0.2.1 已发布到正式目录**（`release\日历待办工具 v0.2.1\`，data 原地未动）：①工具栏顺序「‹ 今天 ›」②年月标题点击选择面板 ③墨刀式色盘（截图实测取色）+ SV 取色面板 ④色块字色自适应 ⑤补齐原型契约（周标题区间/总览 tag/时间控件 showPicker）。**待用户手测 TC-CL-001/010、TC-CAL-003/016/017、TC-DUE-007、TC-IT-013**。

**2026-09-08 v0.2.2 已发布到正式目录**（`release\日历待办工具 v0.2.2\`，data 原地未动）：总览页开放工具栏「+ 新增事项」（所属分类下拉，PRD v1.10 / TC-CL-007），移除「不可新增」标签。待用户手测。**2026-09-08 v0.3.0 已发布到正式目录**（`release\日历待办工具 v0.3.0\`，生产 data 原地未动并已验证自动迁移 v2）：日期类型（班/休/假，PRD v1.12 / TC-DT-001~006，方案经用户确认）+ 横条头尾拖拽 + 待办日数字 + 总览页新增。**待用户手测 TC-DT-001~006、TC-CAL-018、TC-DUE-009**。
...
- **用户正式使用目录（2026-09-08 迁移，当前 v0.3.0）**：`release\日历待办工具 v0.3.0\`（项目根下 release\ 目录，生产数据 data\，82 条事项；.gitignore 已排除 release\）。**今后发新版：新版本目录放 `release\` 下，或仅把新 exe 交付用户替换**；`src-tauri\target\release\` 下的 v0.1.0/v0.2.0 目录均为旧副本（v0.1.0 data 是过期快照），清理须用户确认。
